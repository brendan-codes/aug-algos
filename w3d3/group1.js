class SLL {
    ...
    removeDupesSorted() {
        if (this.head !== null) { // there may be duplicate elements to remove
            let roverA = this.head;
            let roverB = this.head.next;
            while (roverB !== null) {
                if (roverB.data === roverA.data) {
                    roverB = roverB.next;
                    roverA.next.next = null;
                    roverA.next = roverB;
                } else {
                    roverA = roverB;
                    roverB = roverB.next;
    ...
    recursiveMax(current=this.head) {
        if (current === null || current.next === null) {
            return current;
        }
        let restMax = this.recursiveMax(current.next);
        return current.data >= restMax.data ? current : restMax;
    }
